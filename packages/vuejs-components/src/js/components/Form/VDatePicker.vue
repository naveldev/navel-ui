        ...
        <div v-if="calendar.state" class="card">
            ...
            <div class="card-content p-1">
                <div class="calendar">
                    ...
                    <button v-for="(day, index) in dates" :key="index" class="btn btn-soft" :class="dayClassObj(day)" @click.prevent="setSelectedDate(day)">
                        {{ formatDateToDay(day) }}
                    </button>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if (this.modelValue) {
                this.calendar.currDateCursor = new Date(this.modelValue)
                this.calendar.selectedDate = new Date(this.modelValue)
        ...
        methods: {
             ...
             */
            setSelectedDate(day) {
                this.calendar.selectedDate = new Date(day.date)

                if (this.modelValue !== false && !isNaN(this.calendar.selectedDate)) {
                    this.$emit('update:modelValue', this.calendar.selectedDate)
                }
                ...
                if (!day.isCurrentMonth) {
                    ...
                    this.calendar.currDateCursor = new Date(this.calendar.selectedDate.setMonth(selectedMonth))
                }
            },
            /**
        ...
        computed: {
            ...
            dates() {
                ...
                return this.getDateRange(startDate, endDate).map(date => ({
                    ...
                    isSelected: (this.calendar.selectedDate.getDate() == date.getDate() && date.getMonth() == this.calendar.selectedDate.getMonth() && date.getFullYear() == this.calendar.selectedDate.getFullYear())
                }))
            },
            errorMessage() {
